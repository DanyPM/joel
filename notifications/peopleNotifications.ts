  ...
  await dispatchTasksToMessageApps<string>(userUpdateTasks, async (task) => {
    ...
    if (
      ...
      if (!task.userInfo.waitingReengagement) {
        ...
        // waitingReengagement is stale by now — another handler in this run may
        // already have templated this user. On send failure the claim is kept
        // on purpose: pending notifications are already stored and the sweep
        // treats a waiting user without lastReengagementSentAt as immediately
        // due, so the template goes out on the next daily sweep at the latest.
